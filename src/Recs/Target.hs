...
{-# SPECIALISE identifyFromList :: Proxy (a ': as) -> S.Seq Int #-}

instance IdentifyFromList '[] where
  identifyFromList _ = []

{-# SPECIALISE identifyFromList :: Proxy '[] -> S.Seq Int #-}
...
read :: forall c m t.
     (t ~ TupleToList c, IdentifyFromList t, Monad m)
     => Query '(t, '[]) m ()
read = modify (over #reads (<> (identifyFromList $ Proxy @t)))

-- | Write to archetypes with 'c'.
write
  :: forall c m t. (t ~ TupleToList c, IdentifyFromList t, Monad m) => Query '(t, t) m ()
write =
  modify (over #writes (<> (identifyFromList $ Proxy @t))
          . over #reads (<> (identifyFromList $ Proxy @t)))

-- | Read from archetypes that might have 'c'.
optional :: forall c m t.
         (t ~ TupleToList c, IdentifyFromList t, Monad m)
         => Query '(Map Maybe t, '[]) m ()
optional = unsafeCoerce $ read @c @m

without :: forall c m x t.
        (t ~ TupleToList c, IdentifyFromList t, Monad m)
        => Query '( '[], '[] ) m ()
without = undefined
...
type family x1 -⋂-  x2 where
  (a ': as) -⋂- b = If (Contains a b) (a ': (as -⋂- b)) (as -⋂- b)
  '[] -⋂- _ = '[]

-- | Union set of 'x1' and 'x2'.
type family x1 -⋃-  x2 where
  (a ': as) -⋃- b = If (Contains a b) (as -⋃- b) (a ': (as -⋃- b))
  '[] -⋃- b = b
...
type family AssertEachIn c x t :: Constraint where
  AssertEachIn (a ': as) x t =
    (IfC (Contains a x) ()
     (TypeError ('Text "Using undeclared " :<>: ShowAccess t :<>: 'Text " dependency ‘"
                 :<>: ShowType a :<>: 'Text "’ in system.\n"
                 :<>: 'Text "    • Hint: Try adding ‘" :<>: ShowAccess t :<>: 'Text
                 " @(" :<>: ShowType a :<>: 'Text ")’ to your system's dependencies.")))
  AssertEachIn '[] _ _ = ()

type HasBase c = (Elem (PropStore c) ~ c, Get IO (PropStore c), Locate c (IsTuple c))

type SelectAccess l t = If (Equal t 'ReadAccess) (Fst l) (Snd l)

type Has c l t = (HasBase c, AssertEachIn (TupleToList c) (SelectAccess l t) t)
...
type HasBranch c l t =
  (HasBase c, AssertEachIn (Map Maybe (TupleToList c)) (SelectAccess l t) t)

branch :: forall c x. HasBranch c x 'ReadAccess => SystemT x (EcsT IO) ()
branch = undefined

tag :: forall c m x t.
    (Has c x 'WriteAccess, Identifiable c, Monad m)
    => c
    -> SystemT x m ()
tag = undefined
